...
fn all_four_panes_are_configured_in_layout_b() {
    let cmds = build_commands(&Layout::B, &default_agents(), "test-session");
...
fn all_four_panes_are_configured_in_layout_a() {
    let cmds = build_commands(&Layout::A, &default_agents(), "test-session");
    assert!(cmds.len() >= 9);
}
...
fn session_name_appears_in_all_commands() {
    ...
    let cmds = build_commands(&Layout::B, &default_agents(), session);
    for cmd in &cmds {
